...
const AdminRole = () => {
    ...
    const toggleRoleStatus = async () => {
        ...
        try {
            await fetch(`https://naruci.co.rs/api/v1/role/update/${selectedRole.id}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${token}`
    ...
    const handleSave = async () => {
        ...
        const url = editMode
            ? `https://naruci.co.rs/api/v1/role/update/${selectedRole.id}`
            : "https://naruci.co.rs/api/v1/role/create";
