...
const AdminKompanije = () => {
    ...
    const toggleCompStatus = async () => {
        ...
        try {
            await fetch(`https://www.naruci.co.rs/api/v1/kompanija/update/${selectedComp.id}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${token}`
    ...
    const handleSave = async () => {
        ...
        const url = editMode
            ? `https://www.naruci.co.rs/api/v1/kompanija/update/${selectedComp.id}`
            : "https://www.naruci.co.rs/api/v1/kompanija/create";
